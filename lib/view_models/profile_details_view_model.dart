    ...
    try {
      ...
      switch (result) {
        case Ok<UserModel>():
          ...
          final eventsResult = await _eventRepository.getUserEvents(targetUserId);
          switch (eventsResult) {
          ...
          final followUserResult = await _followUserRepository.isFollowing(userId, targetUserId);
          switch (followUserResult) {
            case Ok<FollowUserModel>():
              _followUserModel = followUserResult.value;
              _log.fine('Follow user model loaded: ${_followUserModel!.isActive}');
            case Error<FollowUserModel>():
              _log.warning('Failed to check if following', followUserResult.error);
          }
